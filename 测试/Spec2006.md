...
编译方法见：[qemu编译运行](../Qemu/qemu编译运行.md)
...
单线程，export OMP_NUM_THREADS=1

.f是什么文件？fortran语言

# UnixBench测试

进入UnixBench目录，Makefile查看CC编译器，OPTON优化参数，
```shell
./Run # 启动脚本，qemu只能跑二进制，因此不能直接用qemu-xxx ./Run，需要配置binfmt。
```
